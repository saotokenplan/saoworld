...
为保证代码审查质量和 bisect 可追踪性，单次提交不得超过以下规模上限：

| 指标 | 一般提交上限 | 超大提交阈值（需审批） |
|------|-------------|---------------------|
| 文件数 | ≤ 50 个 | > 50 个 |
| 新增行数 | ≤ 2000 行 | > 2000 行 |
| 变更总行数（新增+删除） | ≤ 3000 行 | > 3000 行 |

### 超过上限的处理

- 达到或超过「一般提交上限」：commit-msg hook 发出警告，提醒拆分
- 达到或超过「超大提交阈值」：commit-msg hook 拦截提交，除非携带 `--no-verify` 并在提交正文中说明拆分理由（仅初始项目脚手架、批量依赖升级等特殊场景允许）

### 拆分原则

提交规模超限时，应按以下优先级拆分：

1. **按服务/模块拆分**：不同微服务、不同工具链分别提交
2. **按层次拆分**：先提交领域模型，再提交仓储层，再提交 API 层，最后提交测试
3. **按主题拆分**：独立功能点各自成提交
4. **规范先行**：规范文档变更单独提交，再提交实现代码

### type 与实际变更一致性

提交信息的 `type` 必须与实际变更内容的主要性质匹配：

| type | 允许的主要变更 | 典型不匹配情形（禁止） |
|------|--------------|---------------------|
| `docs` | 主要为 `.md`/`.yaml` 等文档或配置说明文件，代码行数占比 < 10% | 大量代码文件却标为 docs |
| `feat` | 新增功能、新模块、新接口 | 纯修复或纯文档 |
| `fix` | 缺陷修复为主 | 新功能开发混在 fix 中 |
| `test` | 测试文件为主，代码文件仅为配合测试的少量调整 | 大量业务代码却标为 test |
| `chore` | 构建、依赖、工具链杂项，不影响业务逻辑 | 业务功能变更却标为 chore |
| `refactor` | 结构调整，外部行为不变 | 新增功能混在 refactor 中 |

**校验规则**：commit-msg hook 会根据暂存文件类型推断建议 type，若用户指定的 type 与推断 type 不一致且代码行数占比差距显著，将发出警告；若 `docs` type 下代码文件占比超过 30%，将直接拦截。
