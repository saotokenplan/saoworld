...
当前仓库除六层标准目录外，仍保留以下历史目录：

- `docs/runbook/`
  - 用于承载历史运行手册、门禁排障与运维操作说明
- `docs/packages/`
  - 用于承载历史需求包、专题规范切片和再包装输出

这些目录当前继续保留，但属于过渡例外，而不是新增的一级标准层。默认原则如下：

- 不把它们视为与六层并列的权威目录
- 新增文档优先落回六层标准结构
- 仅当文档天然属于“运行手册”或“专题打包产物”时，才保留在这些目录
- 若后续决定迁移或并回六层，需先更新本规范再执行调整
...
- 对 `runbook/`、`packages/` 这类过渡目录，也应保留局部 `README.md` 说明其来源与定位。
