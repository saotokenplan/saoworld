...
- 适用于需要判断文档层级、权威关系和保留策略的治理场景。
- 适用于新增、整合、迁移或复查现有文档时做全局判断。
- 不替代具体目录规范、变更流程和模板规则。

## 当前定位

- 本文档承担全局视角下的“文档角色说明”和“保留策略说明”。
- 本文档帮助读者理解不同目录的职责分工，但不直接定义新增、迁移或实施顺序。
- 完整目录导航以 `docs/README.md` 为准；最小阅读路径与最小实施顺序以 `docs/00-governance/quick-start.md` 为准。
- 目录规则、变更流程和状态定义以下游治理文档为准。

## 当前判断

- 当前仓库已进入“核心研发完成后、灰度发布前校准”的阶段。
- 当前资产不仅包含需求、规范和研发闭环文档，也已经包含客户端、后端服务、工具链和异步任务代码目录。
- 当前阶段的主要目标是统一入口口径、推进发布闭环与运行时验证，而不是继续把仓库描述为纯文档仓。
- `20-specs/` 仍是执行基线，`30-api/` 是接口参考，`00-governance/` 负责治理导航与状态收口。
...
|---|---|---|---|---|
| `README.md` | 仓库总入口，说明项目现状、根目录结构与阅读顺序 | 应反映当前单仓工程现状，而不是继续描述为纯文档仓 | 保留 | 持续与 `docs/README.md`、`project-status.md` 和根目录实际结构保持同步 |
...
2. 持续同步根 `README.md`、`tools/README.md`、`services/README.md` 与 `project-status.md`，避免 docs 外入口再次漂移。
3. 在后续新增、迁移或大幅改写时，同步执行模板维护检查、入口更新和坏路径复查。
