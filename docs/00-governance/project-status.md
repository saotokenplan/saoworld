...
39. 性能压测工具 perf_test 实现：`tools/perf_test/` 提供核心接口异步压测能力（投票提交 p95 < 300ms、投票查询 p95 < 100ms、内容查询 p95 < 100ms），覆盖 `docs/20-specs/backend-data-spec.md` 规范要求。63 个单元测试通过，4 个门禁注册（G-UNIT-013、G-NONFUNC-001/002/003），Runbook 完整（2026-07-13 02:08 完成）。
40. **建议下一步**：perf_test 接入 CI 流水线（`nightly` 或 `on_release` 触发）、扩展更多核心接口场景（content 灰度发布、world 区域查询、player 个人中心聚合查询），并补充 P4 阶段可观测性基础设施（APM / Distributed Tracing / SLO 指标）。
