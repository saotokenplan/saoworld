...
- **M4 WP3 审核效率监控（进行中）**：指标埋点批（M1 耗时 Histogram / M2 `reviews_total` 补 `review_type` 标签并修复 auto 路径全终局计数 / M3 `metrics.yaml` 标签同步 / M4 规则归因 Counter）已于 2026-07-26 落地，review 服务全量 pytest 87 passed；M5 审核效率看板 JSON 与 M6 review 告警/SLO 已落盘。剩余项：ops 看板三字段（auto_pass_rate / review_p95_minutes / manual_intervention_rate）增补待下一轮（services/ops 改造，分离推进，不阻塞本批）。
- **M4 WP4 发布自动化与周更节奏（未开始，按序）**：自动审核通过内容自动进入发布队列（审核→打包→发布串联）；定义每周区域更新运营流程（规划期草案已就位）。
- **M4 WP5 批量生成能力（未开始，按序）**：确认现有生成链路对批量请求的支持程度，补齐批量入口（如需）。
