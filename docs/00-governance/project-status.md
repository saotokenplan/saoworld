...
- 当前状态：灰度阶段已明确跳过，M4 实施正式启动；现状盘点/差距分析/工作包分解已完成，五项产出文档（模板细化/样本集/阈值预案/看板指标/运营流程草案）全部就位，按工作包顺序逐项推进。WP1 首个实施代码项（A2：`score_region` 字段口径对齐 danger_level/landmarks，修复 F1 缺陷）已于 2026-07-23 落地，场景评分维度恢复生效。
...
| 需求迭代计划文档漂移 | 规划风险 | 中 | 需求迭代计划.md 仍显示"当前阶段：Sprint 9 公测准备"、M1"进行中"，与实际 M4 实施期不符，需重基线（2026-07-24 PM 评审提出） |
| M1 验收口径被绕过 | 战略风险 | 中 | 灰度阶段经决策跳过，M1 原"灰度发布验证通过"验收口径不再成立，需作为产品战略例外正式记录 |
...
- **M4 WP1 模板验收收口（进行中）**：A2（F1 修复）已落地（45 passed）；A1 双源收口迁移依赖真实运行时环境；A3/A4/A5 不依赖运行时可立即推进；A6 需真实 LLM 实测。
- **M4 WP2–WP5（未开始，按序）**：审核规则调优 → 发布自动化 → 批量生成与效率 → 质量看板与运维。
- **运行时验证（并行前置）**：PostgreSQL / Redis / Docker / workers / 客户端三平台导出约 15 项验证，内嵌推进，不再作为独立阻塞阶段。
- **文档重基线（待办）**：`需求迭代计划.md` 重基线为“M4 实施期”、标记灰度跳过、校准 S9/S10/S11；M1 验收绕过作为产品战略例外正式记录。
