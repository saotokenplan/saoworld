...
- **M4 WP4 发布自动化与周更节奏（未开始，按序）**：自动审核通过内容自动进入发布队列（审核→打包→发布串联）；定义每周区域更新运营流程（规划期草案已就位）；为 WP3 闭环后的下一明确节点，依赖运行时/跨服务。
- **M4 WP5 批量生成能力（已发起·在制）**：研发闭环自动化 auto-20260726-1029 已发起 `services/generation` 批量入口与 `test_batch_generation.py`（工作树在制，尚未提交合并）；需与 WP4 发布链路串联后方可常态化周更。
