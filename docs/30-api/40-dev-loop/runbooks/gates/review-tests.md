...
**门禁名称**：Service Unit Tests (review)
**门禁 ID**：G-UNIT-006
**门禁类型**：unit
**触发条件**：
- 路径：`services/review/**`
...
# 清理测试环境后重新运行
cd services/review && pytest --cache-clear

# 单独运行失败的测试，检查数据准备逻辑
cd services/review && pytest tests/test_review_records.py::test_specific_case -xvs
```
...
```bash
# 查看详细断言信息
cd services/review && pytest -v --tb=long

# 运行特定测试文件定位问题
cd services/review && pytest tests/test_review_approve.py -v

# 检查状态机实现
# 文件：services/review/app/domain/models.py
...
cd services/review && pytest -v --tb=short -k "async"

# 确认测试使用正确的 async 标记
# 检查 conftest.py 中的 event_loop fixture
# 文件：services/review/tests/conftest.py
```
...
```bash
# 运行带详细输出的测试
cd services/review && pytest -xvs -k "mock"
...
```bash
# 检查环境变量模板
cat services/review/.env.example

# 确认测试环境变量设置
cd services/review && python -c "from app.core.config import settings; print(settings.model_dump())"

# 设置必需的环境变量后运行
export REVIEW_ENVIRONMENT=test
cd services/review && pytest
```
...
```bash
# 安装开发依赖
cd services/review && pip install -e ".[dev]"

# 验证依赖安装
cd services/review && pip list | grep -E "pytest|sqlalchemy|fastapi"
...
# 检查权限校验逻辑
# 文件：services/review/app/core/auth.py
