...
- 适用于 `docs/30-api/` 目录下所有涉及角色、权限和审计边界的接口参考文档。
- 适用于玩家、运营、审核和系统角色的访问范围梳理与敏感操作约束定义。
- 不替代 `docs/20-specs/` 中的执行规范，也不替代具体接口样例和错误码细表。
...
- `docs/30-api/api-error-codes.md`
  - 细化权限不足、鉴权失败和敏感操作失败时的错误语义。
- `docs/30-api/api-examples-vote.md`
  - 在具体投票接口样例中落地本文件定义的角色访问边界和审计要求。
