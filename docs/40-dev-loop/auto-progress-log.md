...
- **结果**：成功完成
- **关键产出**：
  - 为 8 个后端服务（vote/world/content/generation/review/player/ops/gateway）新增 `app/core/errors.py` 错误码模块
  - 每个服务定义专属错误码常量类和统一的异常抛出函数
  - 实现统一的错误处理中间件，对齐 12-api-design.md 错误响应格式规范
  - 更新 `docs/30-api/api-error-codes.md`，按服务分类整理错误码，对齐实际实现
  - 将 `INTERNAL_ERROR` 从预留移入通用 HTTP 层已落地
  - 更新 `project-status.md`，标记第 20 项（统一错误码）为已完成
  - vote-service 54 个测试全部通过
- **遗留**：各服务 routes.py 尚未迁移到使用统一错误码模块、其他7个服务测试待验证、客户端错误码映射待更新、错误响应格式完全统一待完善
...
- **结果**：成功完成
- **关键产出**：
  - 创建 `docs/30-api/client-ui/` 目录，包含 7 个界面设计文档
  - 主菜单交互稿：布局、元素、交互逻辑
  - 投票界面交互稿：投票面板、结果面板、历史面板
  - 世界地图交互稿：区域卡片、详情、状态样式
  - 任务面板交互稿：任务列表、详情、接取、进度追踪
  - NPC交互稿：NPC列表、对话界面、对话树结构
  - 界面流程图：页面流转、状态迁移、场景加载、错误处理
  - 更新 project-status.md，标记客户端交互稿为已完成
- **遗留**：部分场景文件（.tscn）需在 Godot 引擎中创建、文档需保持与代码同步
