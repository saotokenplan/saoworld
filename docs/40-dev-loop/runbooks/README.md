...
- 适用于 CI 门禁失败排查、灰度发布、全量发布、回滚、部署和迁移等操作型场景。
- 适用于需要查阅可执行步骤和回滚方法的协作方。
- 不替代 `docs/20-specs/` 的正式约束，也不作为新的一级权威层。

## 当前定位

- 本目录是 `docs/40-dev-loop/` 下的正式运行手册子目录，用于承载操作说明与排障手册。
- 本目录已并入研发闭环层，不再作为独立一级目录存在。
- 如与 `docs/20-specs/` 或治理文档冲突，以后者为准。
